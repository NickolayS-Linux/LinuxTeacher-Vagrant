...
Для работы нужно создать каталог Vagrant.

В него скопировать файл Vagrantfile. (Прилагается к ДЗ)

Создать папу **scr** в папке Vagrant. В папку **scr** скопируем созданный скрипт **scr_resizefs.sh** для работы с дисками. (Прилагается к ДЗ)

Установить права на исполнение chmod +x scr_resizefs.sh

Запускаем vagrant up (работа команды записана в журнал wok_upvagrant.logю (Прилагается к ДЗ)
